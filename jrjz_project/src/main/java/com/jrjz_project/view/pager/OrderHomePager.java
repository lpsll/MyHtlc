...
public class OrderHomePager extends BasePager {
...
//    private MibaCategory mibaCategory;
    private String flag;
    public OrderHomePager(Context context,String flag) {
        super(context);
        this.flag=flag;
    }
    ...
    public View initView() {
        View view = View.inflate(context, R.layout.order_status_frag, null);
        return view;
    }
    ...
    public void initData() {
        ...
        String[] str_txt = {"待支付","未完成","待评价","已完成","全部订单"};
        ...
        mLists.add(new Payment_UndoPager(context,orderStatusCate.list.get(0).url));       //待支付
        mLists.add(new Complete_undoPager(context,orderStatusCate.list.get(1).url));     //未完成
        mLists.add(new Comment_undoPager(context,orderStatusCate.list.get(2).url));    //待评价
        mLists.add(new CompletePager(context,orderStatusCate.list.get(3).url));           //已完成
        mLists.add(new AllOrderPager(context,orderStatusCate.list.get(4).url));        //所有订单
